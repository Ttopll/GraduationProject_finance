...
- 家庭成员级别的接口访问控制。
- 固定资产、家庭财务画像、理财建议、数据导出已提供基础接口。

### 2.3 当前明显还没完成的部分

- 微信小程序端未接入。
- Web 管理后台未接入。
- 固定资产、家庭财务画像、理财建议、数据导出已落地基础接口，但距离完整演示版还有差距。
- 规则引擎还只是轻量版，已支持连续多月阈值规则和趋势异常规则，但仍不支持复杂组合条件。
- 已具备定时自动调度，但缺少调度管理、运行监控和更细粒度策略配置。
- 消息中心还是站内消息，没有微信订阅消息、短信、邮件等外部推送。
- 测试覆盖率仍然偏低，当前以 service 级测试为主。
...
- 生成规则通知
- 联动预算预警
- 支持连续 N 个月达到阈值的规则配置与评估
- 支持“当前月相对前 N 个月均值异常增长”的趋势异常规则

限制：

- 规则类型仍以轻量规则为主，当前新增了连续多月阈值和趋势异常规则
- 只支持家庭支出或分类支出
- 仅提供基础定时调度，缺少任务管理和运行监控
- 复杂趋势组合、拐点识别等高级趋势规则仍未实现

### 4.10 已落地基础接口但完成度仍较低的模块

状态：已有实体、接口和基础业务逻辑，但距离答辩演示版还有差距。
...
- `AuthController`
  - 提供登录和获取当前登录用户信息接口。
- `FamilyController`
  - 提供家庭创建、邀请码加入、成员列表、成员管理、户主转交、成员退出和邀请码刷新接口。
- `AccountController`
  - 提供账户创建、修改、启停和家庭账户列表查询接口。
- `CategoryController`
  - 提供分类创建和分类列表查询接口。
- `BudgetController`
  - 提供预算创建、修改、启停、预算列表和预算使用情况统计接口。
...
- `FamilyApiModels`
  - 家庭创建、邀请码加入、成员管理、户主转交和成员退出响应模型。
- `AccountApiModels`
  - 账户创建请求、账户修改请求和账户响应模型。
- `CategoryApiModels`
  - 分类创建请求和分类响应模型。
- `BudgetApiModels`
  - 预算创建、修改请求、预算响应、预算使用率响应模型。
...
- `AccountService`
  - 负责账户创建、修改、启停、按家庭查询账户、按 ID 获取账户，并校验账户归属成员和账单日范围。
- `FamilyService`
  - 负责家庭创建、邀请码加入、成员管理、户主转交、成员退出和邀请码刷新，创建家庭时会自动补一条 `OWNER` 成员记录。
- `CategoryService`
  - 负责分类创建与分类查询。
- `BudgetService`
  - 负责预算创建、修改、启停、预算列表和预算使用情况统计。
